...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      // Organisation assignment always happens afterwards from the Assign tab
      // (internal auditors can be assigned to one or more organisations there).
      // external/admin: never org-scoped — externals pick org per-audit, admins aren't org-scoped.
      const hash = await bcrypt.hash(b.password, 12);
      const rows = await sql`
        INSERT INTO five_s_users(username, full_name, password_hash, role, is_active)
        VALUES(${String(b.username).trim().toLowerCase()}, ${b.full_name}, ${hash}, ${b.role}, TRUE)
